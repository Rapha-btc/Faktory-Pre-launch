...
;; Fee airdrops are separate from token purchase and vesting. (Fees are airdropped for simplicity.)
...
(define-private (distribute-to-holder (entry {owner: principal, seats: uint}))
    (let ((holder (get owner entry))
        ...
        (if (> user-share u0)
                (match (as-contract (stx-transfer? user-share tx-sender holder))
                    success
                        (begin 
                            (print {
